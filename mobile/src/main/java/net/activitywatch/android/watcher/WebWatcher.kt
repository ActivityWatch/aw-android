...
class WebWatcher : AccessibilityService() {
    ...
    // TODO(maintainer): this can include page text/content-descriptions (potentially sensitive
    // browsing content) and is not currently gated to debug builds - buildFeatures.buildConfig
    // isn't enabled in mobile/build.gradle, so there's no BuildConfig.DEBUG to gate on yet.
